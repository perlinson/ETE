...
void CSerialPort::ProcessErrorMessage(char* ErrorText)
{
	...
	//_stprintf_s(Temp, _tcslen(Temp),_T("WARNING:  %s Failed with the following error: \n%s\nPort: %d\n"), (char*)ErrorText, lpMsgBuf, m_nPortNr); 
	//MessageBox(NULL, Temp, _T("Application Error"), MB_ICONSTOP);
	OutputDebugString(_T("Application Error"));
...
{		
	...
	//_tccpy(m_szWriteBuffer, string);
	//m_nWriteSize= _tcslen(string); // add by mrlong
	
	strcpy((char*)m_szWriteBuffer, (char*)string);
...
void CSerialPort::WriteToPort(BYTE* Buffer, int n, int& byteReads)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
	int i;
	...
	BOOL bResult = WriteFile(m_hComm,							// Handle to COMM Port
...
void CSerialPort::SendData(unsigned char * lpszData, const size_t nLength)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, nLength);
	//_tccpy(m_szWriteBuffer, lpszData);
	strcpy((char*)m_szWriteBuffer, (char*)lpszData);
	m_nWriteSize=nLength;
	// set event for write
	SetEvent(m_hWriteEvent);
}

BOOL CSerialPort::RecvData(unsigned char* lpszData, const size_t nSize)
{
	//
	//接收数据
	//
	...
	//while (mylen < nSize)
	//{
	//	BOOL bResult = ReadFile(m_hComm,							// Handle to COMM Port
	//		lpszData,					// Pointer to message buffer in calling finction
	//		nSize,    // add by mrlong
	//		//strlen((char*)port->m_szWriteBuffer),	// Length of message to send
	//		&mylen2,								// Where to store the number of bytes sent
	//		0);	
