...
//Disable warning  "from double to float"
...
public:
  T ** m; // m[i][j] = value at row i and column j
              //           0 <= i < RowCount()
              //           0 <= j < ColCount()
...
private:
  int m_row_count;
  int m_col_count;
  SimpleArray<T *> m_row; // m_row[i][j] = row i and column j
  SimpleArray<T>   m_a; // array of at least Ts
...
Ti_Matrix();
  ...
  Ti_Matrix( // see Ti_Matrix<T>::Create(int,int,int,int) for details
  ...
  Ti_Matrix( const Ti_Matrix& );
  Ti_Matrix(const Ti_Matrix&,int nb,int ne);
  virtual ~Ti_Matrix();
...
public:
	...
	T ArrayDot(int ,const T *,const T*);    
//	int Ti_Matrix<T>::RowReduce( T zero_tolerance,Ti_3dPoint* B,T* pivot  );

//	bool BackSolve( T,int ,const Ti_3dPoint*,Ti_3dPoint*);
	void ON_Array_aA_plus_B( int dim, double a, const T* A, const T* B, T* aA_plus_B );
	void ON_ArrayScale( int dim, double s, const T* A, T* sA );
  
  void EmergencyDestroy(); // call if memory pool used matrix by becomes invalid

  // Ti_Matrix[i][j] = value at row i and column j
  //           0 <= i < RowCount()
  //           0 <= j < ColCount()
  T* operator[](int);
  const T* operator[](int) const;

  Ti_Matrix<T>& operator=(const Ti_Matrix&);
  Ti_Matrix<T> operator+(Ti_Matrix& b) const;
  Ti_Matrix<T> operator-(Ti_Matrix& b) const;

  ////////////////////////////////////////////////
  /////by 杨平海 add down
  bool DoHermite(Ti_Matrix & input,Ti_Matrix & output,int m,int n);
  ////////add up
 ...
 // T ColAdd();
  ...
  void SetDiagonal(T); // sets diagonal value and zeros off diagonal values
  void SetDiagonal(const T*); // sets diagonal values and zeros off diagonal values
  void SetDiagonal(int, const T*); // sets size to count x count and diagonal values and zeros off diagonal values

//  void SetDiagonal(const Ti_SimpleArray<T>& ); // sets size to length X lengthdiagonal values and zeros off diagonal values


  T* GetCol(int n) const;
  T* GetRow(int n) const;
  friend void Cross(T* a, T* b, T* c);
  ...
  // safe arithmetic
  bool Transpose();
  void OutProduct(T* v,int n,T *u, int m);
//  void OutProduct(const VECTOR v1,const VECTOR V2);
  ...
  // must be passed in.
  bool LUFactorLinearSystem(Ti_Matrix&,int* index, int size,T* tmpSize);
   // Description:
  ...
  bool LUSolveLinearSystem(double *x, int size);
 // Description:
  ...
  bool Multiply(const Ti_Matrix&, const Ti_Matrix& );
  bool Add( const Ti_Matrix&, const Ti_Matrix& );
  bool Det(T&result);
  bool Scale( double );
  // Description:
  ...
  // A=U Diag(w)VT
  void SingularValueDecomposition3x3(
  						Ti_Matrix& A,//the matrix to be decompostion
	                    Ti_Matrix& U, 
						T w[3],
					    Ti_Matrix& VT
						);
  // Description:
  // Perform singular value decomposition on a m x n matrix.  This is not
  // done using a conventional SVD algorithm, instead it is done using
  // Orthogonalize3x3 and Diagonalize3x3.  The determinants of both
  // matrices are guaranteed to be positive.
  // A=U Diag(w)VT
...
void  Diagonalize3x3(
 ...
 void Orthogonalize3x3(Ti_Matrix& B);

// Jacobi iteration for the solution of eigenvectors/eigenvalues of a nxn
//  this matrix must be real symmetric matrix. output eigenvalues in w;
// and output eigenvectors in v. Resulting eigenvalues/vectors are sorted
// in decreasing order; eigenvectors are normalized.
  bool  JacobiN(int n, T *w, Ti_Matrix& v);

  bool Matrix3x3ToQuaternion(T quat[4]);
  bool QuaternionToMatrix3x3(T quat[4]);
  // row reduction
  int RowReduce( // returns rank
    double,   // >= 0.0 (zero tolerance for pivot test)
    T&,  // determinant is returned
    T&   // pivot is returned
    ); 

  int RowReduce( // returns rank
    double,        // >= 0.0 (zero tolerance for pivot test)
    T*,       // B[m_row_count] array that is reduced with matrix
    T* = NULL // if not NULL, then smallest pivot is returned
    ); 

  /*int RowReduce( // returns rank
    double,        // >= 0.0 (zero tolerance for pivot test)
    ON_3dPoint*, // B[m_row_count] array that is reduced with matrix
    T* = NULL // if not NULL, then smallest pivot is returned
    ); */
  ...
  int RowReduce( // returns rank
    ...
    int,         // pt_dim
    int,         // pt_stride
    T*,     // pt[m_row_count*pt_stride] array of pt_dim pts
                 // that is reduced with the matrix
    T* = NULL // if not NULL, then smallest pivot is returned
    ); 

 bool
  BackSolve( // matrix must be upper triangular, have unit diagonal,
             // and m_col_count <= m_row_count.  Rows below upper square
             // are ignored.
    T,        // >= 0.0 (zero tolerance)
    int,           // size of B ( >= m_col_count ) B[m_col_count,...,Bsize-1]
                   // are tested to make sure fabs() <= zero tolerance.
    const T*, // B[]
    T*        // X[m_col_count]
      ) const;
 ...
 /*// 
  BackSolve( // matrix must be upper triangular, have unit diagonal,
             // and m_col_count <= m_row_count.  Rows below upper square
             // are ignored.
    double,        // >= 0.0 (zero tolerance)
    int,           // size of B ( >= m_col_count ) B[m_col_count,...,Bsize-1]
                   // are tested to make sure fabs() <= zero tolerance.
    //const ON_3dPoint*, // B[]
    ON_3dPoint*        // X[m_col_count]
      ) const;*/
...
Ti_Matrix<T>::Invert( double zero_tolerance )
{
  ...
  int* col = allocate_integer_vector(0,n-1);
